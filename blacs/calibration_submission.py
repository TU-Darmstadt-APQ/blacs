...
from qtutils import *
from zprocess import TimeoutError, raise_exception_in_thread, ZMQClient
...
class CalibrationSubmission(object):        
    def __init__(self, BLACS, blacs_ui):
        self.inqueue = queue.Queue()
        self.BLACS = BLACS
        self.port = 21348
        self.zmq_client = ZMQClient(allow_insecure=True)
        
        self._ui = UiLoader().load(os.path.join(BLACS_DIR, 'calibration_submission.ui'))
        blacs_ui.calibration.addWidget(self._ui)
    ...
    def mainloop(self):
        self._mainloop_logger = logging.getLogger('BLACS.CalibrationSubmission.mainloop')
    ...
    def check_connectivity(self):
        ...
        if host and send_to_server:       
            ...
            try:
                response = self.zmq_client.get(self.port, host, 'hello', timeout=1)
                self.failure_reason = None
    ...
    def submit_waiting_files(self):
        ...
        while self._waiting_for_submission and success:
            ...
            self.server_online = 'checking'
            try:
                response = self.zmq_client.get(self.port, self.server, data, timeout=1)
                self.failure_reason = None
            except (TimeoutError, gaierror, AuthenticationFailure) as e:
                success = False
                self.failure_reason = str(e)
